      ...
      <div className="max-w-7xl mx-auto">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <button onClick={() => fetchProducts()} className="text-[10px] tracking-[0.25em] uppercase text-foreground border border-border px-8 py-3 hover:border-primary hover:text-primary transition-all cursor-pointer">
              Try Again
            </button>
          </div>
        ) : (
          <>
              ...
              {currentArrivals.map((product) => (
                <div key={product.id} className="group relative flex flex-col" onMouseEnter={() => setHoveredId(product.id)} onMouseLeave={() => setHoveredId(null)}>
                  <div className="relative aspect-[3/4] w-full bg-secondary overflow-hidden border border-border/40 mb-5">
                    <span className="absolute top-3 left-3 z-10 text-[9px] font-medium tracking-[0.15em] uppercase bg-background/90 text-foreground px-2.5 py-1 backdrop-blur-sm border border-border/30">New Addition</span>
            ...
            <div className="mt-20 flex flex-col items-center gap-4 border-t border-border/40 pt-10">
              <div className="flex items-center gap-6">
                <button onClick={() => setCurrentPage(1)} disabled={currentPage === 1} className="text-[10px] tracking-[0.25em] uppercase text-muted-foreground hover:text-foreground disabled:opacity-30 transition-colors cursor-pointer">
                  ‹ Prev
                </button>
                ...
                <button onClick={() => setCurrentPage(2)} disabled={currentPage === 2 || products.length <= 4} className="text-[10px] tracking-[0.25em] uppercase text-muted-foreground hover:text-foreground disabled:opacity-30 transition-colors cursor-pointer">
                  Next ›
                </button>
              </div>
            </div>
