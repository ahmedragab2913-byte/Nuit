    ...
    <div className="pt-28 px-6 lg:px-20 pb-24 min-h-screen bg-background text-foreground" style={sans}>
      ...
      {showOrderSuccess && (
        <div className="mb-10 max-w-5xl bg-emerald-500/5 border border-emerald-500/15 p-4 rounded-sm flex items-center gap-3 text-xs text-emerald-400">
          ...
          <p className="font-light">Thank you! Your order has been placed successfully and is being prepared.</p>
        </div>
      )}

      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div>
          <p className="text-[11px] tracking-[0.45em] uppercase text-primary mb-3">Maison Nuit Club</p>
          <h1 className="text-5xl font-light text-foreground" style={serif}>My Account</h1>
        </div>
        <button
          onClick={handleLogout}
          className="flex items-center gap-2 border border-border/80 hover:border-red-500/30 hover:text-red-400 px-5 py-3 text-[10px] tracking-widest uppercase transition-all cursor-pointer font-medium"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
        ...
        {/* Left Columns: Profile info & Address registry */}
        <div className="lg:col-span-2 space-y-12">
          
          {/* Profile Form */}
          <div className="border-b border-border/40 pb-10">
            <h2 className="text-lg font-light text-foreground uppercase tracking-widest mb-6" style={serif}>Profile Information</h2>
            
            <form onSubmit={handleUpdateName} className="max-w-xl space-y-5">
              <div>
                <label className="block text-[9px] uppercase tracking-wider text-muted-foreground/80 mb-2">Display Name</label>
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="bg-foreground text-background hover:bg-foreground/90 disabled:opacity-40 disabled:hover:bg-foreground px-5 text-[10px] uppercase tracking-wider font-semibold transition-all cursor-pointer flex items-center gap-1.5"
                  >
                    <Save size={12} /> Save
                  </button>
                </div>
                {profileSuccess && (
                  <p className="text-[10px] text-emerald-400 font-light mt-1.5 flex items-center gap-1">
                    <CheckCircle size={10} /> Name updated successfully.
                  </p>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[9px] uppercase tracking-wider text-muted-foreground/50 mb-2">
                    Email Address <span className="text-[8px] italic text-primary/70">(Read Only)</span>
                  </label>
                  <input
                    type="email"
                    disabled
                ...
                <div>
                  <label className="block text-[9px] uppercase tracking-wider text-muted-foreground/50 mb-2">
                    Phone Number <span className="text-[8px] italic text-primary/70">(Read Only)</span>
                  </label>
                  <input
                    type="tel"
                    disabled
          ...
          <div className="border-b border-border/40 pb-10">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-lg font-light text-foreground uppercase tracking-widest" style={serif}>Address Registry</h2>
              {!showAddForm && (
                <button
                  onClick={() => setShowAddForm(true)}
                  className="flex items-center gap-1.5 text-[10px] tracking-wider uppercase text-primary hover:text-primary-foreground transition-colors cursor-pointer"
                >
            ...
            {/* Address Form inline toggle */}
            {showAddForm && (
              <form onSubmit={handleAddAddress} className="bg-secondary/15 border border-border/60 p-6 space-y-4 mb-6">
                <h3 className="text-xs uppercase tracking-wider text-foreground mb-2">New Address Details</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[9px] uppercase text-muted-foreground/85 mb-1">Country</label>
                    <input
                      type="text"
                      disabled
                      value="Egypt"
                  ...
                  <div>
                    <label className="block text-[9px] uppercase text-muted-foreground/85 mb-1">City / Region *</label>
                    <input
                      type="text"
                      required
                      value={city}
                ...
                <div>
                  <label className="block text-[9px] uppercase text-muted-foreground/85 mb-1">Street Address *</label>
                  <input
                    type="text"
                    required
                    value={street}
                    onChange={(e) => setStreet(e.target.value)}
                    placeholder="90th Street, Fifth Settlement"
                    className="w-full bg-background border border-border px-3 py-2 text-xs text-foreground outline-none focus:border-primary placeholder:text-muted-foreground/35"
                  />
                </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <label className="block text-[9px] uppercase text-muted-foreground/85 mb-1">Building *</label>
                    <input
                      type="text"
                      required
                      value={building}
                  ...
                  <div>
                    <label className="block text-[9px] uppercase text-muted-foreground/85 mb-1">Floor</label>
                    <input
                      type="text"
                      value={floor}
                  ...
                  <div>
                    <label className="block text-[9px] uppercase text-muted-foreground/85 mb-1">Apartment</label>
                    <input
                      type="text"
                      value={apartment}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {addresses.map((addr) => (
                <div key={addr.id} className="p-5 border border-border/60 bg-secondary/15 flex flex-col justify-between relative">
                  <div>
                    ...
                    <p className="text-xs text-foreground font-semibold mb-1">Egypt, {addr.city}</p>
              ...
              {addresses.length === 0 && (
                <div className="md:col-span-2 border border-border/30 border-dashed p-10 text-center flex flex-col items-center justify-center">
                  <p className="text-xs text-muted-foreground/70 font-light mb-1">Your address book is empty.</p>
                  <p className="text-[10px] text-muted-foreground/50 font-light mb-4">Please add a default address to checkout faster.</p>
                </div>
              )}
            </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <h2 className="text-lg font-light text-foreground uppercase tracking-widest" style={serif}>Order History</h2>
          ...
          <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
            {orders.map((ord) => (
              <div key={ord.id} className="border border-border/60 bg-secondary/15 p-5 space-y-4">
                    ...
                    <div key={idx} className="flex gap-3 items-center">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-muted-foreground/70">
                          {it.qty} × EGP {it.price.toLocaleString()}
                        </p>
                      </div>
                    </div>
                  ))}
                ...
                {/* Address snapshot description */}
                {ord.address && (
                  <div className="pt-2 border-t border-border/30 text-[10px] text-muted-foreground/80 font-light leading-relaxed">
                    <p className="uppercase tracking-wider text-[8px] text-muted-foreground/50 mb-0.5">Shipping Location</p>
                    <p>{ord.address.building}, {ord.address.street}, {ord.address.city}</p>
                  </div>
                )}

                <div className="pt-3 border-t border-border/30 flex justify-between items-baseline">
                  <span className="text-[10px] text-muted-foreground font-light">Total Paid</span>
                  <span className="text-sm font-semibold text-foreground">
                    EGP {ord.grand_total.toLocaleString()}
                  </span>
                </div>
            ...
            {orders.length === 0 && (
              <div className="border border-border/30 border-dashed p-8 text-center flex flex-col items-center justify-center">
                ...
                <p className="text-xs text-muted-foreground/75 font-light">No orders placed yet.</p>
              </div>
            )}
          </div>
        </div>
